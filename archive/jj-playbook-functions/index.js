...
const REMIND_TOOL = {su: 'routine', sd: 'routine', deep: 'deepwork'};
...
function loggedToday(resp, toolId, tz, todayDate) {
  const hist = (resp && resp.tools && resp.tools[toolId] &&
    resp.tools[toolId].history) || [];
  return hist.some((h) => {
    const ts = h && (h.ts || Date.parse(h.date));
...
exports.dailyReminders = onSchedule('every 5 minutes', async () => {
  ...
  for (const doc of snap.docs) {
    ...
    for (const item of rec.schedule) {
      ...
      const toolId = REMIND_TOOL[item.key];
      if (toolId && resp && loggedToday(resp, toolId, tz, now.date)) continue;
...
exports.onFundamentalDone = onDocumentWritten(COLL + '/{docId}', async (event) => {
  ...
  const pushDoc = await db.collection(COLL).doc('push:' + after.idkey).get();
